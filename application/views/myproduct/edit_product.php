          ...
          <div class="row">
          	<div class="col-sm-6">
               <?= $this->session->flashdata('message'); ?>
          		...
          		<?= form_open_multipart('myproduct/edit_product/'.$product['product_id']); ?>
              ...
              <div class="form-group row">
                  ...
                  <select class="form-control" id="cat_code" name="cat_code" onload ="subcategory()" onchange="subcategory()">
                        <?php foreach ($category as $cat) : ?>
                            <?php
                             ...
                             ?>
                          <option <?php echo ($product['cat_code'] == $cat['cat_code']) ? "selected": "" ?>  value="<?= $cat['cat_code']?>" > <?= $catname['cat_name'];?> </option>
                        <?php endforeach ; ?>
                      </select>
                    ...
                    <?php foreach ($subcategory as $subcat) : ?>
                            <?php
                                   ...
                                   $querySubcat = "    SELECT *
                                              FROM `category`
                                              WHERE `category`.`cat_code` = $cat_code
                                             AND `category`.`subcat_code` = $subcat_code
                                          ";
                                    $subcatname = $this->db->query($querySubcat)->row_array();
                                    

                             ?>
                          <option <?php echo ($product['subcat_code'] == $subcat['subcat_code']) ? "selected": "" ?>  value="<?= $subcat['subcat_code']?>" > <?= $subcatname['subcat_name'];?> </option>
                        <?php endforeach ; ?>
                    </select>
                ...
                <div class="input-group mb-3">
                  ...
                  <input type="text" class="form-control" aria-label="Amount (to the nearest dollar)" name="price" id="price" Placeholder="Harga" value="<?= $product['price']; ?>">
                ...
                <div class="form-group">
                  <textarea class="form-control" id="about" name="about" rows="3" placeholder="Deskripsi Produk"><?= $product['about']; ?></textarea>
